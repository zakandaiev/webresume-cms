...
function route($route, $path_to_include) {
	...
	if($route_parts[0] == '' && count($request_url_parts) == 0) {
		if($_SERVER['REQUEST_METHOD'] == 'GET' && $route != "/install") include_once("views/header.php");
		include_once("$ROOT/$path_to_include");
		exit();
	}
...
function set_csrf() {
	...
	if(isset($_SESSION["csrf_token"]) && !empty($_SESSION["csrf_token"]) && count($_SESSION["csrf_token"]) >= 30) {
...
function is_csrf_valid() {
	if(isset($_SESSION["csrf_token"]) && is_array($_SESSION["csrf_token"]) && isset($_POST["csrf_token"]) && in_array($_POST["csrf_token"], $_SESSION["csrf_token"])) {
		return true;
	}
	return false;
}
